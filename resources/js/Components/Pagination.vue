        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            ...
            <div>
                <nav class="relative z-0 inline-flex shadow-sm">
                    ...
                    <a v-for="(link, i) in data.links" v-if="i > 0 && i < data.links.length - 1" :href="link.url"
                       :class="`${data.current_page == link.label ? 'border-blue-400' : ''} hidden md:inline-flex -ml-px relative items-center px-4 py-2 border border-gray-300 bg-white text-sm leading-5 font-medium text-gray-700 hover:text-gray-500 focus:z-10 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150`">
                        {{ link.label }}
                    </a>

                    <a :href="data.links[data.links.length -1].url" :class="` ${!data.links[data.links.length -1].url ? 'opacity-25' : ''} -ml-px relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm leading-5 font-medium text-gray-500 hover:text-gray-400 focus:z-10 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-gray-100 active:text-gray-500 transition ease-in-out duration-150`" aria-label="Next">
